...
describe('Moonlet - Web 1920 x 1080', () => {
	...
	describe('Landing page', async () => {
		...
		before(
			test(async (browser, opts) => {
				...
				await page.screenshot({
					path: 'e2eoutput/' + pwidth + 'x' + pheight + '-' + slug + '.png'
				});
			})
		);
		...
		it(
			'should have a "Create new wallet" button',
			test(async (browser, opts) => {
				const innerText = await page.evaluate(
					sel => document.querySelector(sel).innerText,
					'button.create-wallet span'
				);
		...
		it(
			'should have a "Restore existing wallet" button',
			test(async (browser, opts) => {
				const innerText = await page.evaluate(
					sel => document.querySelector(sel).innerText,
					'button.restore-wallet span'
				);
	...
	describe('Create Wallet page', async () => {
		...
		before(
			test(async (browser, opts) => {
				...
				await page.screenshot({
					path: 'e2eoutput/' + pwidth + 'x' + pheight + '-create-wallet-page-1-start.png'
				});
			})
		);

		it(
			'should have the proper header section title of "Create New Wallet"',
			test(async (browser, opts) => {
				const innerText = await page.evaluate(
					sel => document.querySelector(sel).innerText,
					'header section > span.title'
				);
		...
		it(
			'should be on step 1 of the wallet creation',
			test(async (browser, opts) => {
				const innerText = await page.evaluate(
					sel => document.querySelector(sel).innerText,
					'div.create-wallet-step1 h5'
				);
		...
		it(
			'should have a "Reveal Secret Phrase" button',
			test(async (browser, opts) => {
				const innerText = await page.evaluate(
					sel => document.querySelector(sel).innerText,
					'div.word-list-overlay button span'
				);
		...
		describe('1 - Click on "Reveal Secret Phrase"', async () => {
				...
				test(async (browser, opts) => {
					...
					await page.screenshot({
						path:
                            ...
                            'x' +
                            pheight +
                            '-create-wallet-page-2-click-reveal-button.png'
					});
				})
			);

			it(
				'should reveal 12 seed words',
				test(async (browser, opts) => {
			...
			it(
				'should have a "Confirm Backup" button',
				test(async (browser, opts) => {
					const innerText = await page.evaluate(
						sel => document.querySelector(sel).innerText,
						'div.create-wallet-step1 > button span'
					);
				...
				test(async (browser, opts) => {
					...
					await page.screenshot({
						path:
                            ...
                            'x' +
                            pheight +
                            '-create-wallet-page-3-0-click-confirm-backup-button.png'
					});
				})
			);

			it(
				'should raise an alert "Re-confirm Backup"',
				test(async (browser, opts) => {
					const innerText = await page.evaluate(
						sel => document.querySelector(sel).innerText,
						'h2.mdc-dialog__header__title span'
					);
				...
				test(async (browser, opts) => {
					...
					await page.screenshot({
						path:
                            ...
                            'x' +
                            pheight +
                            '-create-wallet-page-3-1-a-click-re-confirm-backup-NO-button.png'
					});
				})
			);
				...
				test(async (browser, opts) => {
					...
					await page.screenshot({
						path:
                            ...
                            'x' +
                            pheight +
                            '-create-wallet-page-3-2-b-click-re-confirm-backup-YES-button.png'
					});
				})
			);

			it(
				'should be on step 2 of the wallet creation',
				test(async (browser, opts) => {
					const innerText = await page.evaluate(
						sel => document.querySelector(sel).innerText,
						'div.create-wallet-step2 h5'
					);
		...
		describe('4 - Validate Secret Phrase', async () => {
				...
				test(async (browser, opts) => {
					await page.screenshot({
						path:
                            ...
                            'x' +
                            pheight +
                            '-create-wallet-page-4-validate-words.png'
					});
				})
			);

			it(
				'validation should present the same words as before',
				test(async (browser, opts) => {
			...
			it(
				'clicking words at random should raise an error',
				test(async (browser, opts) => {
			...
			it(
				'clicking words in the correct order shoule enable the "Validate Secret Phrase" button',
				test(async (browser, opts) => {
				...
				test(async (browser, opts) => {
					...
					await page.screenshot({
						path:
                            ...
                            'x' +
                            pheight +
                            '-create-wallet-page-5-setup-password.png'
					});
				})
			);

			it(
				'should be on step 3 of the wallet creation',
				test(async (browser, opts) => {
					const innerText = await page.evaluate(
						sel => document.querySelector(sel).innerText,
						'div.create-wallet-step3 h5'
					);
